...
Pickup_Layer::Pickup_Layer(engine::ref<Player>& player_ref, engine::ref<engine::game_object>& item_ref) :
	m_2d_camera(-1.6f, 1.6f, -0.9f, 0.9f), m_player(player_ref), m_item(item_ref) {
	...
	if (m_item->type() == 0) {
		...
		pickup_text += "First Aid Spray?";
		end_transform = glm::rotate(end_transform, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	} else if (m_item->type() == 2) {
		pickup_text += "Handgun Ammo?";
	...
	if (!m_player->is_inventory_full()) {
		// If inventory isn't full
		m_button_manager->create_text_button("Yes", 0.9f, {-0.1f, -0.51f}, {1.f, 1.f, 1.f, 0.5f}, {1.f, 1.f, 1.f, 1.f}, m_text_manager, [this]()
		{
			engine::application::window().hide_mouse_cursor();
			m_button_manager->set_active(false);

			// Add item to the player's inventory.
			m_player->addToInventory(m_item->type());
			m_item->set_remove(true); // Mark item for removal.
		...
		m_button_manager->create_text_button("No", 0.9f, {0.1f, -0.51f}, {1.f, 1.f, 1.f, 0.5f}, {1.f, 1.f, 1.f, 1.f}, m_text_manager, [this]()
		{
			engine::application::window().hide_mouse_cursor();
			m_button_manager->set_active(false);
		...
		// If inventory is full, don't allow the player to add another item to the inventory.
		pickup_text = "Your inventory is full.";
		m_button_manager->create_text_button("Back", 0.9f, {0.0f, -0.51f}, {1.f, 1.f, 1.f, 0.5f}, {1.f, 1.f, 1.f, 1.f}, m_text_manager, [this]()
		{
			engine::application::window().hide_mouse_cursor();
			m_button_manager->set_active(false);
	...
	m_button_manager->create_text_button(pickup_text, 1.0f, {0.0f, -0.4f}, {1.f, 1.f, 1.f, 1.0f}, {1.f, 1.f, 1.f, 1.f}, m_text_manager, []()
	                                     {}
	);
...
void Pickup_Layer::attach_scene_transition(const engine::timestep& time_step) {
...
void Pickup_Layer::detach_scene_transition(const engine::timestep& time_step) {
	transition_timer -= time_step;
...
void Pickup_Layer::on_update(const engine::timestep& time_step) {
...
void Pickup_Layer::on_event(engine::event& event) {
	if (!(attach_transition_in_progress || detach_transition_in_progress)) {
